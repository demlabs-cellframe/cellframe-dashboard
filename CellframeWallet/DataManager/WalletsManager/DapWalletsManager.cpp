...
void DapWalletsManager::walletsListReceived(const QVariant &rcvData)
{
    ...
    for(const QJsonValue &value: std::as_const(walletArray))
    {
        ...
        if(tmpObject.contains(Dap::JsonKeys::NAME) && tmpObject.contains(Dap::JsonKeys::PATH))
        {
            ...
            bool isPrimatyPath = (tmpWallet.path == Dap::WalletDefines::WalletStorage::WALLET_PATH);
            {
                WalletPathInfo pathInfo = walletPaths.contains(walletName)? walletPaths.value(walletName) : WalletPathInfo();

                if(isPrimatyPath)
                    pathInfo.priorityPath = Dap::WalletDefines::WalletStorage::WALLET_PATH;
                else
                    pathInfo.secondPath = Dap::WalletDefines::WalletStorage::WALLET_NODE_PATH;
...
void DapWalletsManager::updateListWallets()
{
    ...
    pathList.append(Dap::WalletDefines::WalletStorage::WALLET_PATH);
    pathList.append(Dap::WalletDefines::WalletStorage::WALLET_NODE_PATH);
