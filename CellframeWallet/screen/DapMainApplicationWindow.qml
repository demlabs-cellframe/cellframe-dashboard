    ...
    Connections
    {
        ...
        function onSigVersionInfo(versionResult)
        {
            ...
            if(jsonDocument.hasUpdate && jsonDocument.message === "Reply version")
                logicMainApp.rcvNewVersion(settingsModule.walletVersion, jsonDocument)
            else if(!jsonDocument.hasUpdate && jsonDocument.message === "Reply version")
                console.log("You have the latest version installed. Current version: ", settingsModule.walletVersion)
            else
