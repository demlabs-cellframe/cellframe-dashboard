...
QtObject {
    ...
    function updateFiltrApps(status)
    {
        ...
        else if(status === qsTr("Unverified"))
        {
            for(var i = 0; i < dapModelPlugins.count; i++ )
            {
    ...
    function searchElement(text)
    {
        ...
        for(var i = 0; i < temporaryModel.count; i++)
        {
            ...
            {
                if(dAppsScreen.currentFiltr === qsTr("Both"))
                {
                    listModelApps.append(temporaryModel.get(i))
                }
                else if(dAppsScreen.currentFiltr === qsTr("Verified") && temporaryModel.get(i).verifed === "1")
                {
                    listModelApps.append(temporaryModel.get(i))
                }
                else if(dAppsScreen.currentFiltr === qsTr("Unverified") && temporaryModel.get(i).verifed === "0")
                {
                    listModelApps.append(temporaryModel.get(i))
                }
            }
