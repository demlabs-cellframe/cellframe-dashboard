  ...
  if (!match.hasMatch()){
      ...
      if(cmd == "Connect"){
          ...
          QString walletName, net, addr, value, tokenName, id, hashTx, certType,
                  certName, timeStaking, reinvest, stakeNoBaseFlag = "", srv_uid, uint;
          ...
          while(matchIt.hasNext())
          {
              ...
              else if(match.captured(1) == "uint")
                  uint = match.captured(2);
          ...
          if(!s_id.isEmpty() && s_id.filter(id).length()){
              if(cmd == "GetWallets")
                  ...
                  doc = createCondTx(net, tokenName, walletName, certName, value, uint, srv_uid);
