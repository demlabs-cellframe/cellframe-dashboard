...
create_link()
{
    ...
    REDIRECT_PATH_ON_SERVER=${DPATH}/${REDIRECT_DIR}
      #printf "REDIRECT_PATH_ON_SERVER: %s\n" "${REDIRECT_PATH_ON_SERVER}"
    FILE_ON_SERVER=$(basename "$2")
      #printf "FILE_ON_SERVER: %s\n" "${FILE_ON_SERVER}"
    FILE_PATH_ON_SERVER=${DPATH}/${3}/\"$(basename "$2")\"
      #printf "FILE_PATH_ON_SERVER: %s\n" "${FILE_PATH_ON_SERVER}"
    FILE_PATH_ON_SERVER_FOR_REDIRECT=${DPATH}/${3}/$(basename "$2")
      #printf "FILE_PATH_ON_SERVER_FOR_REDIRECT: %s\n" "${FILE_PATH_ON_SERVER_FOR_REDIRECT}"
    
    DEST_PATH_ON_SERVER=${REDIRECT_PATH_ON_SERVER%/*}
      #printf "DEST_PATH_ON_SERVER: %s\n" "${DEST_PATH_ON_SERVER}"
    RELEASE_TYPE=$(basename "$REDIRECT_DIR")
      #printf "RELEASE_TYPE: %s\n" "${RELEASE_TYPE}"
    
    # LATEST_NAME=${CI_PROJECT_NAME}-${RELEASE_TYPE}.${FILE_PATH_ON_SERVER_FOR_REDIRECT##*.}
    ...
    #ssh -i ${KEY} -q ${CRDT} ls "${FILE_PATH_ON_SERVER_FOR_REDIRECT}" && echo "File exists, creating symlink" ||  { echo "File does not exist";  exit 2; }
      printf "%s\n" "ssh -i ${KEY} -q ${CRDT} ls ${FILE_PATH_ON_SERVER_FOR_REDIRECT}" 
    ...
    #ssh -i ${KEY} -q ${CRDT} if test -d "${DEST_PATH_ON_SERVER}"/"${RELEASE_TYPE}"\; then rm -r "${DEST_PATH_ON_SERVER}"/"${RELEASE_TYPE}"\; fi
      ...
      printf "%s\n" "ssh -i ${KEY} -q ${CRDT} "ssh -i ${KEY} -q ${CRDT} if test -d "${DEST_PATH_ON_SERVER}"/"${RELEASE_TYPE}"\; then rm -r "${DEST_PATH_ON_SERVER}"/"${RELEASE_TYPE}"\; fi""

    #LINK_CREATE=(ssh -i "${KEY}" -q "${CRDT}" "cd "${DEST_PATH_ON_SERVER}" && ln -sf ./"${FILE_ON_SERVER}" ./"${RELEASE_TYPE}"")
      printf "%s\n" "ssh -i ${KEY} -q ${CRDT} cd ${DEST_PATH_ON_SERVER} &&  ln -sf ./${FILE_ON_SERVER}  ./${RELEASE_TYPE}"
    #"${LINK_CREATE[@]}"
}
