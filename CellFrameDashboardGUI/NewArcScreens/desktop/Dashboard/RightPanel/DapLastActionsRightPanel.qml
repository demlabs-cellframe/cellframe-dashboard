...
DapLastActionsRightPanelForm
{
    ...
    Connections
    {
        ...
        onAllWalletHistoryReceived:
        {
            if (walletHistory.length !== lastHistoryLength)
            {
                if (walletHistory.length < lastHistoryLength)
                {
                    console.warn("ERROR! walletHistory.length < lastHistoryLength",
    ...
    {
        target: dashboardTab
        onComponentOnCompleted:
        {
            if(ok)
            {
                lastHistoryLength = 0
                updateWalletHisory()
                if (!updateTimerLastActions.running)
                    updateTimerLastActions.start()
            }
            else
                updateTimerLastActions.stop()
        }
    }

    Timer {
        id: updateTimerLastActions
        interval: 1000; running: true; repeat: true
        onTriggered:
        {
            console.log("trigger", currentRightPanel, lastActionsWallet)
            if(currentRightPanel === lastActionsWallet)
            {
                updateWalletHisory()
    ...
    function updateWalletHisory()
    {
        if (globalLogic.currentIndex >= 0)
        {
            ...
            globalLogic.getWalletHistory(globalLogic.currentIndex, _dapModelWallets)
        }
    }
